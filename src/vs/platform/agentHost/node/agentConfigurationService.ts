...
import { copilotModelTeamRuntimeSchema } from '../common/copilotModelTeam.js';
...
export class AgentConfigurationService extends Disposable implements IAgentConfigurationService {
	...
	constructor(
		...
		const modelTeamRuntimeSchema = copilotModelTeamRuntimeSchema.toProtocol();
		const agentMergeSchema = agentMergeRootConfigSchema.toProtocol();
		this._stateManager.rootState.config = {
			schema: {
				type: 'object',
				properties: { ...existing?.schema.properties, ...ownSchema.properties, ...sandboxSchema.properties, ...copilotCliSchema.properties, ...modelTeamRuntimeSchema.properties, ...agentMergeSchema.properties },
			},
