...
import { ThinkingLevelConfigKey } from './agentModelConfiguration.js';
...
export const CopilotModelTeamSupportConfigKey = 'copilotModelTeamSupport';
export const CopilotModelTeamAppliedConfigKey = 'copilotModelTeamApplied';
export const CopilotModelTeamLeadAppliedConfigKey = 'copilotModelTeamLeadApplied';
...
export const copilotModelTeamSchema = createSchema({
	...
	[CopilotModelTeamAppliedConfigKey]: schemaProperty<ICopilotModelTeam | Record<string, never>>({
		...teamProperty,
		title: localize('modelTeam.appliedTitle', "Applied Model Team"),
		sessionMutable: false,
		readOnly: true,
	}),
	[CopilotModelTeamLeadAppliedConfigKey]: schemaProperty<ModelSelection>({
		...modelProperty,
		title: localize('modelTeam.appliedLeadTitle', "Applied Lead Model"),
		readOnly: true,
	}),
});

export const copilotModelTeamRuntimeSchema = createSchema({
	[CopilotModelTeamSupportConfigKey]: schemaProperty<number>({
		type: 'number',
		title: localize('modelTeam.supportTitle', "Native Model Team Contract Version"),
		enum: [0, 1],
		default: 0,
		readOnly: true,
	}),
...
export function omitCopilotModelTeamConfig<T>(values: Record<string, T>): Record<string, T> {
	...
	delete result[CopilotModelTeamAppliedConfigKey];
	delete result[CopilotModelTeamLeadAppliedConfigKey];
	delete result[CopilotModelTeamSupportConfigKey];
	return result;
}
...
export function parseCopilotModelTeam(value: unknown): ICopilotModelTeam | undefined {
	...
	if (worker.id === 'auto' || scout?.id === 'auto') {
		throw new Error(localize('modelTeam.explicitHelpers', "Choose a specific model for each helper. Auto is only available for the Lead."));
	}
...
export function validateCopilotModelTeam(team: ICopilotModelTeam, models: readonly IAgentModelInfo[]): void {
	...
	for (const [role, selection] of [
		...
		for (const [key, value] of Object.entries(selection.config ?? {})) {
			if (key !== ThinkingLevelConfigKey) {
				throw new Error(localize('modelTeam.unsupportedHelperOption', "Model teams do not support the helper option '{0}'. Remove it before applying the team.", key));
			}
