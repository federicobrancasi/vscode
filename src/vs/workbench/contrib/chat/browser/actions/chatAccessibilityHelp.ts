...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat' | 'quickChat' | 'editsView' | 'agentView', keybindingService: IKeybindingService, supportsFileReferences: boolean, isSessionsWindow: boolean = false, stickyPromptHeaderShown: boolean = false, sessionStatusPillsSupported: boolean = type === 'panelChat' || type === 'agentView', sessionArchiveNudgeShown: boolean = false, sessionArchiveActionWording = ChatSessionArchiveActionWording.Archive): string {
	const content = [];
	if (isSessionsWindow) {
		content.push(localize('chat.testApp.help', "After app UI changes, Test App appears to the right of the status pills above the chat input. Press Shift+Tab from the input to focus it, then Enter or Space to ask the agent to review the diffs, test the app UI, fix issues found, and retest. Press Tab from Test App to reach App Testing Options, then Enter or Space to open the menu. Test with Subagent requests a separate tester for that run only; if unavailable, the main agent explains this and tests itself. Escape dismisses the menu. Existing tool permissions still apply."));
	}
