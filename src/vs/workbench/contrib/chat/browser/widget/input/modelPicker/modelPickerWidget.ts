...
export class ModelPickerWidget extends Disposable {
	...
	show(anchor?: HTMLElement): void {
		...
		const getAdditionalActionGroups = () => this._workspaceTrustInitialized && this._unavailableReason() === undefined
			? this._delegate.getAdditionalActionGroups?.() ?? []
			: [];
		...
		if (additionalContent?.replaceModelList || (this.isTabbedPickerEnabled() && !this.isRestrictedMode() && (models.length > 0 || placeholders.length > 0))) {
			...
			this._showTabbedPicker(anchorElement, {
				...
				getAdditionalActionGroups,
				selectionPresentation: this._delegate.selectionPresentation,
				additionalContent,
		...
		const createItems = () => buildModelPickerItems({
			models,
			selectedModelId: this._selectedModel?.identifier,
			suppressModelSelection: !!this.selectionPresentation,
			additionalActionGroups: getAdditionalActionGroups(),
			recentModelIds: this._languageModelsService.getRecentlyUsedModelIds().filter(id => !this._languageModelsService.isModelHidden(id)),
			pinnedModelIds: this._languageModelsService.getPinnedModelIds().filter(id => !this._languageModelsService.isModelHidden(id)),
			controlModels: controlModelsForTier,
			currentVSCodeVersion: this._productService.version,
		...
		if (this._delegate.selectionPresentation) {
			pickerDisposables.add(runOnChange(this._delegate.selectionPresentation, () => {
				const items = createItems();
