...
import type { IModelPickerAdditionalContent, IModelPickerDelegate } from './modelPickerActionItem.js';
...
import { getModelPickerAccessibilityProvider, prependModelPickerActionGroups } from './modelPickerItems.js';
...
export interface ITabbedModelPickerContext extends Pick<IModelPickerDelegate, 'getAdditionalActionGroups' | 'selectionPresentation'> {
...
export class TabbedModelPicker extends Disposable {
	...
	private readonly _selectionPresentationListener = this._register(new MutableDisposable());
	private readonly _additionalActionDisposables = this._register(new DisposableStore());
	...
	constructor(
		...
		const onDidHide = () => {
			...
			this._selectionPresentationListener.clear();
			this._additionalActionDisposables.clear();
	...
	show(anchor: HTMLElement, context: ITabbedModelPickerContext): void {
		...
		this._showCurrent();
		this._selectionPresentationListener.value = context.selectionPresentation && this.isVisible
			? runOnChange(context.selectionPresentation, () => {
				this._widget.refreshActiveList({ preserveHover: true });
	...
	private _showCurrent(initialFilterValue?: string): void {
		...
		this._widget.show<IActionWidgetDropdownAction>({
			...
			createActionList: activeTab => {
				...
				const modelItems = this._searchVisible
				...
				const additionalActionGroups = current.getAdditionalActionGroups?.();
				for (const group of additionalActionGroups ?? []) {
					for (const action of group.actions) {
						if (action.hover?.disposable) {
							this._additionalActionDisposables.add(action.hover.disposable);
						}
					}
				}
				const items = !modelItems.length && destination.placeholders.length
					? modelItems
					: prependModelPickerActionGroups(modelItems, additionalActionGroups);
				return {
					items,
	...
	private _isAutoSelected(context: ITabbedModelPickerContext): boolean {
		...
		return !context.selectionPresentation?.get() && !!selected && isAutoModel(selected);
	}
	...
	private _createModelItem(
		...
		const checkedModelId = context.selectionPresentation?.get() ? undefined : context.selectedModelId;
