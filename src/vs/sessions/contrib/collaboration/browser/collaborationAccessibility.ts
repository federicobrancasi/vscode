...
export class CollaborationAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider | undefined {
		...
		const content = [
			...
			localize('collaboration.help.sidebar', "Agent Collab in the Sessions sidebar opens this view. Before a room is open it shows a form for the shared goal, the working folder, and how many agents to use with a model for each; shared rules, the Git branch, and optional run limits are under Advanced. Rooms you already created are listed in the Sessions sidebar: expand Agent Collab to open one directly. Once a room is open the main pane contains the shared conversation and message input, and Room Settings opens a right panel whose run controls and tabs stay pinned above an independently scrolling body. With Room Settings focused, use Left Arrow to widen the open side panel and Right Arrow to narrow it. On narrow windows it opens a drawer; Escape closes the drawer. Closing the panel preserves approval answers and model choices. Returning through Agent Collab after opening a participant restores the selected room."),
			localize('collaboration.help.navigation', "Use Tab and Shift+Tab to move between room controls, the panel tabs, messages, and the message input. The side panel begins with Start, Pause and Stop All, then a tab list: one tab for each agent, then Rules, then Approvals. Use Left and Right Arrow, Home, and End to move between tabs; a tab is badged when it holds something needing attention. The shared message list supports arrow keys and Page Up or Page Down. An agent tab opens that agent's existing room chat for detailed activity and changes, and keeps its Stop and Retry actions. Needs Attention in the header selects the tab that owns the pending request or failure."),
