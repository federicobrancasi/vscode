...
export class CollaborationAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider | undefined {
		...
		const content = [
			...
			localize('collaboration.help.sidebar', "Agent Collab in the Sessions sidebar opens this view. The main pane contains the shared conversation and message input. Room Settings opens an independently scrolling right panel with the room picker, New Room, agents, configuration, and creation fields. With Room Settings focused, use Left Arrow to widen the open side panel and Right Arrow to narrow it. On narrow windows it opens a drawer; Escape closes the drawer. Closing the panel preserves approval answers and model choices. Returning through Agent Collab after opening a participant restores the selected room."),
			localize('collaboration.help.navigation', "Use Tab and Shift+Tab to move between room controls, participants, approvals and questions, messages, and the message input. The shared message list supports arrow keys and Page Up or Page Down. Participant buttons open the agent's existing room chat for detailed activity and changes. Stop and Retry remain available in each agent row. Needs Attention in the header opens pending requests or failed peers even when Room Settings is hidden."),
			localize('collaboration.help.models', "Each numbered Copilot has its own searchable model menu in Room Settings, both before creating the room and afterwards. Select a model with the keyboard and close the menu with Escape. Changing a model does not start work. A busy peer continues its current turn; its new model applies on the next turn. Pending changes and model errors are shown in that peer's row."),
			localize('collaboration.help.approvals', "Approvals and questions appear directly in the room with the peer's name and the actual tool input, result, or question. Review the content, then use Allow, Reject, or the exact options offered by the host. You can answer forms and review plans without opening each peer. The card waits for the host's confirmation; rejected or timed-out responses show an error and can be retried. Input drafts and focus survive unrelated activity updates."),
			...
			localize('collaboration.help.delivery', "A normal message to a busy agent waits for its next turn; Steer Agents instead sends live guidance. Pause holds pending messages, and an in-progress Stop does not admit new work. Retry Delivery retries a saved pending, cancelled, or failed human message without posting it twice. Messages from agents cannot restart a stopped room."),
