...
export class CollaborationAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider | undefined {
		...
		const content = [
			localize('collaboration.help.overview', "You are in a collaboration room. You and the Copilot agents share a conversation and a goal. Agents keep working until you pause or stop them, and post findings after completing meaningful work."),
			localize('collaboration.help.sidebar', "Agent Collab in the Sessions sidebar opens this view. Before a room is open it shows a form for the shared goal, the working folder, and how many agents to use with a model for each; shared rules and the Git branch are under Advanced. Rooms you already created are listed in the Sessions sidebar: expand Agent Collab to open one directly. Once a room is open this view contains only the shared conversation and message input. Room settings move to the side panel, where Room Settings takes the place Changes and Files hold in an ordinary session; its run controls and tabs stay pinned above an independently scrolling body. Room Settings in the header, and Needs Attention, both bring that panel forward. Escape in the panel returns to the conversation. Hiding the panel preserves approval answers and model choices. Returning through Agent Collab after opening a participant restores the selected room."),
			localize('collaboration.help.navigation', "Use Tab and Shift+Tab to move between the panel tabs, messages, and the message input. The side panel begins with a tab list of four tabs: Run, Agents, Rules, and Approvals. Use Left and Right Arrow, Home, and End to move between them; a tab is badged when it holds something needing attention. Run offers only the actions the current state allows: Start before a run, then Pause and Stop All. Needs Attention appears when something is waiting. Agents lists every peer with its model and state. Each peer offers Stop while it can still work, otherwise Resume, named Retry when it stopped because it failed; Remove retires it from the roster after confirming, keeping its posts in the room. Add Agent at the end of the list gives the room another peer. A peer's name opens that agent's own room chat once it has taken a turn. The shared message list supports arrow keys and Page Up or Page Down. Needs Attention selects the tab that owns the pending request or failure."),
			...
			localize('collaboration.help.messages', "Messages identify their author. Send addresses every Copilot when there are no mentions, or only the participants mentioned with @. Running peers receive human guidance; stopped peers keep it pending until Resume. Use the reply action to respond to a particular message. Structured results show their outcome, evidence, published patches, and Pending, Verified, or Rejected state. Review Result lets you choose Verify Result or Reject Result and enter the evidence you checked. A review is an attributed room record; it does not apply or merge a patch, and it does not wake an agent."),
