...
suite('Collaboration accessibility', () => {
	...
	test('help documents peer messaging, run controls and workspace boundaries', () => {
		...
		assert.deepStrictEqual({
			type: provider.options.type,
			verbosity: provider.verbositySettingKey,
			hybrid: content.includes('coordinator supervises without blocking worker progress'),
			mainTabs: content.includes('main tab list has Coordinator and Activity'),
			activityRouting: content.includes('bypassing the coordinator'),
			coordinatorAuthority: content.includes('cannot Stop, Resume, Add, Remove'),
			mentions: content.includes('workers mentioned with @'),
			...
			trust: content.includes('exact local worktrees of the coordinator and workers'),
			...
			hybrid: true,
			mainTabs: true,
			activityRouting: true,
			coordinatorAuthority: true,
			mentions: true,
			keyboard: true,
			pause: true,
			changes: true,
