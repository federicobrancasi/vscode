...
import { ModelPickerWidget } from '../../../../workbench/contrib/chat/browser/widget/input/modelPicker/modelPickerWidget.js';
import { getCompactModelPickerIcon } from '../../../../workbench/contrib/chat/browser/widget/input/modelPicker/modelProviderIcons.js';
import { createModelConfigurationActions, ILanguageModelChatMetadata, ILanguageModelChatMetadataAndIdentifier, ILanguageModelsService, IModelConfigurationAccess } from '../../../../workbench/contrib/chat/common/languageModels.js';
...
type IModelTeamRolePicker = Pick<ModelPickerWidget, 'onDidChangeSelection' | 'onDidClose' | 'setSelectedModel' | 'show' | 'showConfiguration' | 'dispose'>;
...
export function getModelTeamModels(models: readonly ILanguageModelChatMetadataAndIdentifier[], role: ModelTeamRole): readonly ILanguageModelChatMetadataAndIdentifier[] {
	return models.filter(model => model.metadata.isUserSelectable !== false
		&& !model.metadata.isBYOK
		&& ILanguageModelChatMetadata.getAgentHostByokManageModelsIdentifier(model.metadata) === undefined
		&& (role === 'lead' || (!isAutoModel(model) && !model.metadata.configurationSchema?.required?.some(key => {
			const property = model.metadata.configurationSchema?.properties?.[key];
			return property?.group !== MODEL_CONFIG_GROUP_EFFORT || property.readOnly;
		}))))
		.map(model => {
			...
			const properties = Object.fromEntries(Object.entries(schema.properties ?? {}).filter(([, property]) => property.group === MODEL_CONFIG_GROUP_EFFORT));
			return {
				...model,
				metadata: {
					...model.metadata,
					configurationSchema: { ...schema, properties, ...(schema.required ? { required: schema.required.filter(key => Object.hasOwn(properties, key)) } : {}) },
				},
			};
		});
}
...
function memberStatus(member: ISessionModelTeamMember): string {
	switch (member.status) {
		case SessionStatus.InProgress: return localize('modelTeam.memberWorking', "Working");
		case SessionStatus.NeedsInput: return localize('modelTeam.memberInput', "Approval or input needed");
		case SessionStatus.Error: return localize('modelTeam.memberError', "Last assignment failed");
		default: return member.enabled ? localize('modelTeam.memberIdle', "Waiting for Lead") : localize('modelTeam.memberPaused', "Paused");
	}
}
...
export class SessionModelTeamPicker extends Disposable {
	...
	private readonly _configurationState = derivedOpts<Omit<ISessionModelTeamState, 'members'> | undefined>({ owner: this, equalsFn: equals }, reader => {
		const state = this._state.read(reader);
		if (!state) {
			return undefined;
		}
		const { members: _members, ...configuration } = state;
	...
	private _setup: IModelTeamSetup | undefined;
	...
	constructor(
		...
		super();
		this._createPicker = createPicker ?? (delegate => instantiationService.createInstance(ModelPickerWidget, delegate));
	...
	getAdditionalContent(): IModelPickerAdditionalContent | undefined {
		...
		return state && (state.supported || state.selection || state.error || state.members?.length) && !state.loading
			? {
				...
				replaceModelList: !!state.selection,
				render: state.selection ? (container, context) => this._render(container, context)
	...
	private _renderHeader(parent: HTMLElement, popup: ModelTeamPopup): IDisposable {
		...
		store.add(autorun(reader => {
			const state = this._state.read(reader);
			toggle.checked = !!state?.selection;
			toggle.disabled = this._saving.read(reader) || (!state?.supported && !state?.selection);
	...
	private _render(parent: HTMLElement, popup: ModelTeamPopup): IDisposable {
		...
		store.add(autorun(reader => {
			...
			const roles: ModelTeamRole[] = state.selection.scoutModelId ? ['lead', 'worker', 'scout'] : ['lead', 'worker'];
			...
			for (const role of roles) {
				...
				card.appendChild($('.model-team-role-description')).textContent = role === 'lead'
					...
					: role === 'worker' ? localize('modelTeam.workerAction', "Implements and tests") : localize('modelTeam.scoutAction', "Read-only research");
				...
				reader.store.add(button.onDidClick(() => void this._chooseModel(role, popup)));
				const model = getModelTeamModels(models, role).find(model => model.identifier === selection?.modelId);
				...
				reader.store.add(reasoning.onDidClick(() => void this._chooseModel(role, popup, true)));
				if (role !== 'lead') {
			...
			toggle.label = state.selection.scoutModelId ? localize('modelTeam.removeScout', "Remove Scout") : localize('modelTeam.addScout', "{0} Add Scout", '$(add)');
			toggle.enabled = enabled;
			reader.store.add(toggle.onDidClick(() => void this._toggleScout(popup)));
			reader.store.add(this._renderHistory(panel, popup, true));
			panel.appendChild($('.model-team-notice')).textContent = state?.error ?? localize('modelTeam.usageNotice', "Applies on the next request. Additional agents use credits.");
		}));
		store.add(toDisposable(() => {
			panel.remove();
	...
	private _renderHistory(parent: HTMLElement, popup: ModelTeamPopup, pausedOnly = false): IDisposable {
		...
		store.add(autorun(reader => {
			history.hidden = !this._state.read(reader)?.members?.some(member => !pausedOnly || !member.enabled);
		}));
	...
	private _renderMemberActions(parent: HTMLElement, role: SessionModelTeamRole, popup: ModelTeamPopup, showRole = false, pausedOnly = false): IDisposable {
		...
		store.add(autorun(reader => {
			...
			entry.hidden = !member || (pausedOnly && member.enabled);
			open.enabled = !!member;
			reset.enabled = state?.supported === true && !!member && !!this._provider?.resetModelTeamMember && !isActiveSessionStatus(member.status) && !this._saving.read(reader);
	...
	private _getConfiguration(role: ModelTeamRole, modelId: string): Readonly<Record<string, unknown>> | undefined {
		if (this._setup?.[role]?.modelId === modelId) {
			return this._setup[role]?.configuration;
		}
		const state = this._state.get();
		if (role === 'lead') {
	...
	private async _toggleTeam(enabled: boolean, popup: ModelTeamPopup): Promise<void> {
		const context = this._context.get();
		if (!context || this._saving.get() || this._rolePicker.value) {
			return;
		}
		popup.hide();
		try {
			...
			if (!enabled) {
				...
				for (const role of roles) {
					const selection = this._setup[role];
					const model = getModelTeamModels(this._models.get(), role).find(model => model.identifier === selection?.modelId);
					if (!model || !isSelectableConfiguration(model, selection?.configuration)) {
						if (!await this._pickRole(context, role, popup)) {
							return;
						}
					}
				}
	...
	private async _toggleScout(popup: ModelTeamPopup): Promise<void> {
		if (!this._state.get()?.selection?.scoutModelId) {
			await this._chooseModel('scout', popup);
			return;
		}
		const context = this._context.get();
	...
	private async _chooseModel(role: ModelTeamRole, popup: ModelTeamPopup, configure = false): Promise<void> {
		const context = this._context.get();
		if (!context || this._rolePicker.value || this._saving.get()) {
			return;
		}
		popup.hide();
		try {
			await this._pickRole(context, role, popup, configure);
		} catch (error) {
			this._reportError(error);
		} finally {
			if (!this._store.isDisposed && sameContext(context, this._context.get())) {
	...
	private async _pickRole(context: ISessionModelTeamContext, role: ModelTeamRole, popup: ModelTeamPopup, configure = false): Promise<boolean> {
		const store = new DisposableStore();
		...
		const models = () => [...getModelTeamModels(this._models.get(), role)];
		...
		try {
			...
			store.add(picker.onDidClose(() => closed.complete()));
			if (configure) {
				picker.showConfiguration(popup.anchor, MODEL_CONFIG_GROUP_EFFORT);
			} else {
				picker.show(popup.anchor);
			}
			await closed.p;
	...
	private async _apply(context: ISessionModelTeamContext, leadModelId: string, team: ISessionModelTeam | undefined, leadModelConfiguration?: Readonly<Record<string, unknown>>): Promise<void> {
		...
		if (!(team ? getModelTeamModels(models, 'lead') : models).some(model => model.identifier === leadModelId)
			|| (team && !getModelTeamModels(models, 'worker').some(model => model.identifier === team.workerModelId))
			|| (team?.scoutModelId && !getModelTeamModels(models, 'scout').some(model => model.identifier === team.scoutModelId))) {
